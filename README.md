...
| ✅ 删除二次确认 | `/rm` 或 `/删除` 后，需要回复 `/确认删除` |
| 🔄 索引修复 | 启动重建、`/repair` 或 `/修复` 手动修复，可选后台一致性检查 |
...
|----------|----------|------|
| `/nashelp` | `/nas帮助` | 显示帮助 |
| `/ls [路径]` | `/列表 [路径]`、`/查看 [路径]` | 查看目录内容 |
| `/get 文件` | `/获取 文件`、`/下载 文件` | 发送已保存文件，支持 `分类/文件名` |
...
| 文件被外部删除 | `/get`、`/search`、`/recent` 等操作会懒清理脏索引 |
