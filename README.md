...
| 🔄 **索引修复** | `/repair` 清理脏记录、补齐缺失记录、更新变化记录 |
| 🏥 **状态统计** | `/du` 查看磁盘空间、数据库大小、文件统计、索引状态 |
...
| `nas` | ❓ 显示帮助 |
| `ls [路径]` | 📂 查看目录内容 |
| `tree [路径] [深度]` | 🌳 查看目录树（默认深度 2，最大 5） |
| `get <文件名>` | 📤 发送已保存的文件（支持 `分类/文件名`） |
| `search <关键词>` | 🔍 模糊搜索文件 |
| `recent [数量]` | 🕘 查看最近保存的文件（默认 10，最大 30） |
| `rm <文件名>` | 🗑️ 删除文件（管理员，需二次确认） |
| `rename <源> <新名称>` | ✏️ 重命名文件（管理员，仅改文件名） |
| `du` | 💾 空间与状态统计 |
| `repair` | 🛠️ 修复索引（清理脏记录、补齐缺失记录，仅管理员） |
...
**Q: 🧹 如何修复索引？**
A: 🧹 执行 `/repair`（仅管理员）—— 清理脏记录、补齐缺失记录、更新变化记录。
