...
> 🚀 **AstrBot 私聊文件自动归档插件** — 基于 SQLite WAL 索引 + 文件系统 Single Source of Truth 架构，支持自动分类、去重、搜索、标签、备注、批量操作、ZIP 导出与索引修复。
...
├── config.py        ← 配置归一化
├── constants.py     ← 版本和内部目录常量
├── command_args.py  ← 命令文本解析
├── file_services.py ← 文件查找、导出与发送
...
默认启用精简模式，只保留归档、浏览、获取、搜索、移动、确认删除、状态与索引修复。管理员外部路径访问默认开启；普通用户始终受 `save_root` 和公开目录约束。关闭 `simple_mode` 可恢复标签、备注、重复审计、批处理和 ZIP 导出。
...
选择器支持 `tag:标签`、`category:分类`、`search:关键词`、`path:目录`。开启 `allow_all_users` 后，普通用户只能在 `public_read_dir` 内使用查看、搜索、获取、查看标签和查看备注类命令。
